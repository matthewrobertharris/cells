...
from terrain.map import ScalarMapLayer, ObjectMapLayer
...
# Maximum framerate of the game
FPS = 200

# Difference in height that cells can't reach
HEIGHT_DIFF = 4
...
class Agent(object):
    ...
    def __str__(self):
        return "Agent(team %i, loaded %r, energy %i, x=%d, y=%d)" % (self.team, self.loaded, self.energy, self.x, self.y)
...
class Plant(object):
    ...
    def __str__(self):
        return "Plant(Efficiency %i, x=%d, y=%d)" % (self.eff, self.x, self.y)
...
class Display(object):
    ...
    def update(self, terr, pop, plants, agent_map, plant_map, energy_map,
        ...
        if show_energy:
            ...
            g = numpy.uint64(g) << numpy.uint64(8)
...
def main():
    ...
    try:
        ...
        with open('default.cfg', 'wt') as configfile:
